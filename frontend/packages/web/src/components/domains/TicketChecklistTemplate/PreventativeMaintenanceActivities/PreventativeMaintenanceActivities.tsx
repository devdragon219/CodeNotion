...
import { TicketChecklistTemplatePreventativeMaintenanceActivitiesProps } from './PreventativeMaintenanceActivities.types';

export const TicketChecklistTemplatePreventativeMaintenanceActivities = ({
  control,
  currentActivities = [],
  errors,
  mode,
}: TicketChecklistTemplatePreventativeMaintenanceActivitiesProps) => {
  ...
  const activities = useMemo(() => {
    ...
    return catalogueType.activities.filter(
      ({ activityId, activityType }) =>
        activityType === CatalogueTypeActivityType.PlannedMaintenance && !currentActivityIds.includes(activityId),
    );
  }, [catalogueType, currentActivities]);

  return (
    <Grid2 container spacing={{ xs: 2, sm: 3 }} sx={{ height: '100%' }}>
      <SectionTitle value="ticket_checklist_template.section_title.preventative_maintenance_activities" />
      {mode === FormMode.Edit && errors.preventative?.activities?.message && (
        <Grid2 size={12}>
        ...
        <Controller
          ...
          render={({ field }) => (
            <TransferList
              ...
              rows={activities}
              titles={{
                left: 'ticket_checklist_template.section_title.select_preventative_maintenance_activities',
