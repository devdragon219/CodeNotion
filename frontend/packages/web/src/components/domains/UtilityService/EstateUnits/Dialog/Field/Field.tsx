...
export const EstateUnitsField = ({ control, currentEstateUnits, currentEstates }: EstateUnitsFieldProps) => {
  const {
    i18n: { language },
    t,
  } = useTranslation();
  const [queryState] = useGetAllEstateUnitsQuery({
    variables: {
      where: {
        estate: {
          id: {
            in: currentEstates.map(({ id }) => id),
          },
        },
        id: {
          nin: currentEstateUnits.map(({ id }) => id),
        },
      },
    },
  });
