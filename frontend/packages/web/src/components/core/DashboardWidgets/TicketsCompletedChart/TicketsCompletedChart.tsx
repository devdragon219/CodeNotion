...
import { ChecklistTicketsCountLineChartDataPointFragment } from '../../../../gql/RealGimm.Web.ChecklistTicketsCountLineChartDataPoint.fragment';
import { useGetChecklistTicketsCountLineChartQuery } from '../../../../gql/RealGimm.Web.Ticket.operation';
...
export const TicketsCompletedChartWidget = ({ useMockData }: DashboardWidgetProps) => {
  const {
    i18n: { language },
    t,
  } = useTranslation();
  ...
  const [queryState] = useGetChecklistTicketsCountLineChartQuery({
    pause:
  ...
  const data = useMemo(() => {
    ...
    const parseDataPoints = (dataPoints: { dataPoint: ChecklistTicketsCountLineChartDataPointFragment }[]) => ({
    ...
    const statistics = queryState.data?.ticket.checklistTicketsCountLineChart;
    switch (period) {
  ...
  }, [customChartType, language, period, queryState.data?.ticket.checklistTicketsCountLineChart, useMockData]);
    ...
    <DashboardWidgetContainer isLoading={queryState.fetching} useBoxShadow={false}>
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ justifyContent: 'space-between', px: 2 }}>
        <Typography variant="h4" sx={(theme) => ({ color: theme.palette.grey[700] })}>
          {t('component.dashboard_widget.tickets_completed_chart.title')}
        </Typography>
        <Stack direction="row" spacing={2} sx={{ justifyContent: 'flex-end' }}>
          ...
          {period === DashboardWidgetPeriod.Custom && (
            <>
              <DateField
                label={t('component.dashboard_widget.tickets_completed_chart.start_date')}
              ...
              />
              <DateField
                label={t('component.dashboard_widget.tickets_completed_chart.end_date')}
                value={customEndDate}
              ...
              />
              <SelectField
                label={t('component.dashboard_widget.tickets_completed_chart.chart_type')}
      ...
      <Box sx={{ px: 2 }}>
        {data.dates.length === 0 ? (
          ...
          <DashboardChart
            type="line"
            series={[data.preventative, data.onTrigger].map((data, i) => ({
              name:
                i === 0
                  ? t('component.dashboard_widget.tickets_completed_chart.tickets_preventative')
                  : t('component.dashboard_widget.tickets_completed_chart.tickets_on_condition'),
              data,
