...
import { useGetTicketsExcludedFromMaintenanceContractExpensesLineChartQuery } from '../../../../gql/RealGimm.Web.Ticket.operation';
import { TicketsAmountChartDataPointFragment } from '../../../../gql/RealGimm.Web.TicketsAmountChartDataPoint.fragment';
...
export const TicketsExcludedFromMaintenanceContractExpensesChartWidget = ({ useMockData }: DashboardWidgetProps) => {
  const {
    i18n: { language },
    t,
  } = useTranslation();
  ...
  const [queryState] = useGetTicketsExcludedFromMaintenanceContractExpensesLineChartQuery({
    pause:
  ...
  const data = useMemo(() => {
    ...
    const parseDataPoints = (dataPoints: { dataPoint: TicketsAmountChartDataPointFragment }[]) => ({
      excluded: dataPoints.map(({ dataPoint }) => dataPoint.excludedAmount),
      nonExcluded: dataPoints.map(({ dataPoint }) => dataPoint.nonExcludedAmount),
    });

    const statistics = queryState.data?.ticket.ticketsAmountLineChart;
    switch (period) {
  ...
  }, [customChartType, language, period, queryState.data?.ticket.ticketsAmountLineChart, useMockData]);
    ...
    <DashboardWidgetContainer isLoading={queryState.fetching} useBoxShadow={false}>
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ justifyContent: 'space-between', px: 2 }}>
        <Typography variant="h4" sx={(theme) => ({ color: theme.palette.grey[700] })}>
          {t('component.dashboard_widget.tickets_excluded_from_maintenance_contract.title.total_expenses')}
        </Typography>
        ...
        {data.dates.length === 0 ? (
          ...
          <DashboardChart
            ...
            series={[data.nonExcluded, data.excluded].map((data, i) => ({
              name:
                i === 0
                  ? t('component.dashboard_widget.tickets_excluded_from_maintenance_contract.non_excluded_expenses')
                  : t('component.dashboard_widget.tickets_excluded_from_maintenance_contract.excluded_expenses'),
              data,
