...
import { useGetTicketsCountMonthlyChartQuery } from '../../../../gql/RealGimm.Web.Ticket.operation';

const mockData: Record<number, number[]> = {
  2024: [80, 95, 90, 65, 110, 140, 180, 200, 100, 90, 180, 140],
};

export const TicketsCountChartWidget = ({ useMockData }: DashboardWidgetProps) => {
  const {
    i18n: { language },
    t,
  } = useTranslation();
  ...
  const [queryState] = useGetTicketsCountMonthlyChartQuery({
  ...
  const data = useMemo(() => {
    ...
    const statistics = queryState.data?.ticket.ticketsCountMonthlyChart;
    return (
      statistics?.reduce<Record<number, number[]>>((acc, it) => {
        ...
        return {
          ...acc,
          [it.year]: acc[it.year].map((month, idx) => (idx === it.month - 1 ? it.dataPoint.ticketsCount : month)),
        };
      }, {}) ?? {}
    );
  }, [queryState.data?.ticket.ticketsCountMonthlyChart, useMockData]);
    ...
    <DashboardWidgetContainer isLoading={queryState.fetching} useBoxShadow={false}>
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ justifyContent: 'space-between', px: 2 }}>
        <Typography variant="h4" sx={(theme) => ({ color: theme.palette.grey[700] })}>
          {t('component.dashboard_widget.tickets_count_chart.title')}
        </Typography>
        <Stack direction="row" spacing={2} sx={{ justifyContent: 'flex-end' }}>
          <SelectField
            label={t('component.dashboard_widget.tickets_count_chart.years')}
            multiple
            max={3}
