...
import { IssuesStatusLineChartDataPointFragment } from '../../../../gql/RealGimm.Web.IssuesStatusLineChartDataPoint.fragment';
import { useGetIssueTicketsStatusLineChartQuery } from '../../../../gql/RealGimm.Web.Ticket.operation';
...
const mockData = {
  ...
  assigned: [0, 50, 40, 70, 60, 20, 30, 10, 40, 30, 60, 20],
  completed: [50, 100, 90, 120, 110, 70, 80, 60, 90, 80, 110, 70],
  inProgress: [100, 150, 140, 170, 160, 120, 130, 110, 140, 130, 160, 120],
  new: [150, 200, 190, 220, 210, 170, 180, 160, 190, 180, 210, 170],
  resolved: [200, 250, 240, 270, 260, 220, 230, 210, 240, 230, 260, 220],
};

export const TicketsStatusChartWidget = ({ useMockData }: DashboardWidgetProps) => {
  const {
    i18n: { language },
    t,
  } = useTranslation();
  ...
  const [queryState] = useGetIssueTicketsStatusLineChartQuery({
    pause:
  ...
  const data = useMemo(() => {
    ...
    const parseDataPoints = (dataPoints: { dataPoint: IssuesStatusLineChartDataPointFragment }[]) => ({
      assigned: dataPoints.map(({ dataPoint }) => dataPoint.assignedCount),
      completed: dataPoints.map(({ dataPoint }) => dataPoint.completedCount),
      inProgress: dataPoints.map(({ dataPoint }) => dataPoint.inProgressCount),
      new: dataPoints.map(({ dataPoint }) => dataPoint.newCount),
      resolved: dataPoints.map(({ dataPoint }) => dataPoint.resolvedCount),
    });

    const statistics = queryState.data?.ticket.issuesStatusLineChart;
    switch (period) {
  ...
  }, [customChartType, language, period, queryState.data?.ticket.issuesStatusLineChart, useMockData]);
    ...
    <DashboardWidgetContainer isLoading={queryState.fetching} useBoxShadow={false}>
        ...
        <Stack direction="row" spacing={2} sx={{ justifyContent: 'flex-end' }}>
          ...
          {period === DashboardWidgetPeriod.Custom && (
            <>
              ...
              />
              <SelectField
                label={t('component.dashboard_widget.tickets_status.chart_type')}
      ...
      <Box sx={{ px: 2 }}>
        {data.dates.length === 0 ? (
          ...
          <DashboardChart
            type="line"
            series={[data.new, data.assigned, data.inProgress, data.resolved, data.completed].map((data, i) => {
              const getName = () => {
                switch (i) {
                  case 0:
                    return t('component.dashboard_widget.tickets_status.status_new');
                  case 1:
                    return t('component.dashboard_widget.tickets_status.status_assigned');
                  case 2:
                    return t('component.dashboard_widget.tickets_status.status_in_progress');
                  case 3:
                    return t('component.dashboard_widget.tickets_status.status_resolved');
                  default:
                    return t('component.dashboard_widget.tickets_status.status_completed');
                }
              };
              ...
              const getColor = () => {
                switch (i) {
                  ...
                  case 3:
                    return theme.palette.danger[300];
                  default:
