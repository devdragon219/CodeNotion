...
public class ILIARateRecapByCityReportGenerator : ReportGenerator
{
  ...
  private readonly IStringLocalizer<ILIARateRecapByCityReportGenerator> _localizer;
  ...
  public static Guid StaticId => Guid.Parse("743e4119-1355-441b-9c90-4ae2a03027e4");
  ...
  private async Task<IEnumerable<MainRowByCity>> GetInnerDataAsync(Dictionary<string, ReportGeneratorFilter> filters)
  {
    if (int.TryParse(filters[FilterManagementSubjectId].StringValue, out int managementSubjectId))
    {
      ...
      var results = grouped.Select(main => new MainRowByCity(
        managementSubjects
