...
public sealed class DeleteTests : EmptyDbWebTest
{
  ...
  {
    // Arrange
    UtilityService utilityServiceToDelete;
    ...
    await using (var scope = Provider.CreateAsyncScope())
    {
      ...
      utilityServiceToDelete = await repository.AddAsync(utilityServiceFaker.Generate());
    }

    var deleteMutation = QueryRequestBuilder.New()
      .SetQuery(Mutation)
      .SetVariableValue("id", utilityServiceToDelete.Id)
    ...
    await using (var scope = Provider.CreateAsyncScope())
    {
      ...
      var deletedUtilityService = await repository.SingleOrDefaultAsync(new GetByIdSpec<UtilityService>(utilityServiceToDelete.Id));
      Assert.Null(deletedUtilityService);
    }
  }
}
