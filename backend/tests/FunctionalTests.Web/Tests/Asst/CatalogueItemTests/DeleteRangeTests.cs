...
public sealed class DeleteRangeTests : EmptyDbWebTest
{
  ...
  public async Task Should_DeleteRange()
  {
    ...
    using (var scope = Provider.CreateScope())
    {
      ...
      itemsToDeleteId = items.Select(item => item.Id).Take(2).ToArray();
      itemsToKeepIds = items.Select(item => item.Id).Except(itemsToDeleteId).ToArray();
    }

    var deleteMutation = QueryRequestBuilder.New()
      .SetQuery(Mutation)
      .SetVariableValue("ids", itemsToDeleteId)
